...
% load('E_Pain_adj_ROI.mat')
% load('BehaviorTally.mat')
% clearvars -except fps conv_x alignTime Formalin start_mark BehTime
% 
% conv_x = 60;
% alignF = [14897, 12201, 12872, 3594];
% newStart = [];
% alignT = (alignF-newStart)./fps/conv_x;
% NewTime = round( repmat(BehTime, 1, size(alignT,2)) + repmat(alignT, size(BehTime, 1), 1) );
% 
% newFormalin = zeros( max( NewTime((:, 4))));
% 
% for ti = 1:4
% newFormalin(NewTime(:,ti), ti) = Formalin(:,ti);
% end
